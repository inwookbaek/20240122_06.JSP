...
<body>
	<%= id %> = <%= request.getAttribute("name") %><br>
	<hr />
	<h3>EL(표현식언어)</h3>
	1. 요청URI = ${pageContext.request.requestURI}<br>
	2. name속성 = ${ requestScope.name }
	3. id(파라미터) = ${ param.id }<br>
	<hr />
	
	표현식<br>
	1. 스크립트릿  : <%= id %> = <%= request.getAttribute("name") %><br>
	2. EL(표현언어): ${ param.id } = ${ requestScope.name }<br>
